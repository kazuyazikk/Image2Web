from css_generator import group_elements
...
def generate_html (elements):
    ...
    html_elements = []
    groups = group_elements(elements) #Start grouping the elements
    
    already_have_navbar = False #Make sure that we only have one navbar
    
    for g_idx, group in enumerate(groups):
        group_tags = [f'<div class="group_{g_idx}">'] #Start of the group wrapper
        ...
        for i, el in enumerate(group):
            ...
            #Might want to include heading tags (h1,h2,h3,etc.)
                ...
                #Changed from Span to h6 as requested by expert, Queency Koh
                tag = f'<h6 class="text-base {class_name}">{label_description(el["label"])}</h6>'
            elif el["label"] == "navbar" and already_have_navbar == False:
                #Put the navbar under body
                nav_class_name = f"{el['label']}_{i}"
                tag = f'''
                <nav class="navbar-base {nav_class_name}">
                    <ul>
                        <li><a href="#">Home</a></li>
        ...
        group_tags.append("</div>") #Close the group wrapper
