...
IMAGE_DIR = "C:/Users/CJREBORN2ND/Desktop/SCHOOL/4thYear/Image2Web-FINAL/dataset_root/wireframes/"
ANNOTATION_DIR = "C:/Users/CJREBORN2ND/Desktop/SCHOOL/4thYear/Image2Web-FINAL/data/annotations/"
OUTPUT_DIR = "C:/Users/CJREBORN2ND/Desktop/SCHOOL/4thYear/Image2Web-FINAL/predictions_json/"
...
def is_text_next_to_checkbox(checkbox_box, text_box, max_distance=300, alignment_tolerance=200):
    ...
    # More flexible horizontal alignment (text to the right of checkbox)
    horizontal_aligned = (
            abs(cb_center_y - text_center_y) <= max(cb_height / 2 + text_height / 2,
                                                    alignment_tolerance) and  # More flexible vertical alignment
            text_x1 >= cb_x1 - 20 and  # Allow some overlap/gap (was cb_x2 - 10)
            text_x1 - cb_x2 <= max_distance  # Distance threshold
    )

    # More flexible vertical alignment (text below checkbox)
    vertical_aligned = (
            abs(cb_center_x - text_center_x) <= max(cb_width / 2 + text_width / 2,
                                                    alignment_tolerance) and  # More flexible horizontal alignment
            text_y1 >= cb_y1 - 20 and  # Allow some overlap/gap (was cb_y2 - 10)
            text_y1 - cb_y2 <= max_distance  # Distance threshold
    )

    # Additional check: text to the left of checkbox (less common but possible)
    left_aligned = (
    ...
    # Additional check: text above checkbox (less common but possible)
    above_aligned = (
...
def group_checkbox_with_text(final_boxes):
    """Group checkboxes with nearby text elements."""
    grouped_boxes = []
    used_indices = set()
...
# ==========================
# NMS + Prediction Function
# ==========================
def get_predictions(image_path):
    """Run model on an image and return final NMS-filtered predictions."""
...
# ==========================
def predictions_to_json_enhanced(final_boxes, save_path, orig_w, orig_h):
    """Save predictions with checkbox-text grouping."""
...
# ==========================
# Debug Function for Tuning Parameters
# ==========================
def debug_checkbox_text_association(final_boxes, image_name, orig_w, orig_h):
    """Debug function to visualize checkbox-text associations and help tune parameters."""
...
def visualize_prediction(image_name: str, save_json: bool = True, debug_associations: bool = False):
    ...
    if save_json:
        ...
        predictions_to_json_enhanced(final_boxes, save_path, pil_img.width, pil_img.height)
...
if __name__ == "__main__":
    ...
    visualize_prediction("20250814_002903.jpg", save_json=False, debug_associations=True)

    # You can also test different parameters by modifying the function call:
    # Test with more lenient parameters for difficult cases:
    # In the is_text_next_to_checkbox function, try:
    # - max_distance=150 (increase from 120)
    # - alignment_tolerance=80 (increase from 50)